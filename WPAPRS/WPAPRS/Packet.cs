 ...
 */
    ...
    public class Packet
    {
        ...
        private static readonly int AX25_CRC_CORRECT = 0xF0B8;
        private static readonly int CRC_CCITT_INIT_VAL = 0xFFFF;
        private static readonly int MAX_FRAME_SIZE = // not including delimiting flags
        ...
        public static readonly int AX25_CONTROL_APRS = 0x03;
        public static readonly int AX25_PROTOCOL_COMPRESSED_TCPIP = 0x06;
        public static readonly int AX25_PROTOCOL_UNCOMPRESSED_TCPIP = 0x07;
        public static readonly int AX25_PROTOCOL_NO_LAYER_3 = 0xF0; // used for APRS

        private readonly int[] crc_ccitt_tab = new int[] {
        ...
        public byte[] BytesWithCRC()
        {
            ...
            Array.Copy(packet, arr, size);
            return arr;                  // trim the checksum
        }

        public byte[] BytesWithoutCRC()
        {
            byte[] arr = new byte[size - 2];
            Array.Copy(packet, arr, size - 2);
            return arr;                   // trim the checksum
        }
        ...
        static void AddCall(byte[] bytes, int offset, String call, bool last)
        {
            int i;
            ...
            for (i = 0; i < call.Length; i++)
            {
                ...
                {
                    ssid = int.Parse(call.Substring(i + 1));
        ...
        public void Parse()
        {
            ...
            payload = new byte[size - 2 - offset];
            Array.Copy(packet, 0, payload, 0, size - 2 - offset);

            //payload = Arrays.copyOfRange(packet, offset, size - 2); // chop off CRC
        }

        private void CrcCcittUpdate(byte b)
        {
        ...
        public override String ToString()
        {
            ...
            if (path != null) foreach (String via in path)
                {
                    builder.Append(',');
                    builder.Append(via);
                }
